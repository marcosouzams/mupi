...
interface CasesPageClientProps {
  translations: CasesTranslations;
  featuredCases: Case[];
  allCases: Case[];
}

// Helper function para obter dados traduzidos de um case
const getTranslatedCaseData = (caseItem: Case, t: CasesTranslations) => {
  const caseData = t.cases.caseData[caseItem.id];
...
// Sectors data com contagem
const getSectorsWithCount = (allCases: Case[], t: CasesTranslations) => {
  const sectors = [
    { id: 'todos', count: allCases.length },
    { id: 'eventos', count: allCases.filter(c => c.sector === 'eventos').length },
    { id: 'cartorio', count: allCases.filter(c => c.sector === 'cartorio').length },
    { id: 'agricultura', count: allCases.filter(c => c.sector === 'agricultura').length },
    { id: 'farmaceutico', count: allCases.filter(c => c.sector === 'farmaceutico').length },
    { id: 'orgaos-publicos', count: allCases.filter(c => c.sector === 'orgaos-publicos').length },
    { id: 'educacao', count: allCases.filter(c => c.sector === 'educacao').length },
    { id: 'saude', count: allCases.filter(c => c.sector === 'saude').length },
  ];

  return sectors.map(sector => ({
    ...sector,
    name: t.cases.sectors[sector.id] || sector.id,
  }));
};

export const CasesPageClient = ({ translations: t, featuredCases, allCases }: CasesPageClientProps) => {
  ...
  const translatedSectors = getSectorsWithCount(allCases, t);
  
  // Filtrar cases baseado no setor selecionado
  const filteredCases = selectedSector === 'todos' 
    ? allCases 
    : allCases.filter(c => c.sector === selectedSector);
  ...
  return (
    <>
                    ...
                    <div className="relative z-10 flex items-center h-full">
                      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                        <div className="max-w-lg">
                          ...
                          <h2 className="text-2xl lg:text-3xl font-urbancat-st font-bold text-white leading-tight mb-4">
      ...
      <section 
        ref={ctaSectionRef}
        className="relative py-20 px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-r from-[#5667fe] to-[#4a56e2]"
      >
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <div className={`flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center transition-all duration-1000 ease-out delay-400 ${
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center space-x-3 bg-white text-[#5667fe] hover:bg-white/90 px-6 py-3 sm:px-8 sm:py-4 rounded-lg transition-all duration-300 font-medium group text-sm sm:text-base"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center space-x-3 bg-transparent hover:bg-white/10 text-white border-2 border-white/50 hover:border-white px-6 py-3 sm:px-8 sm:py-4 rounded-lg transition-all duration-300 font-medium text-sm sm:text-base"
            >
